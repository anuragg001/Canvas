...
export default function Home() {
  ...
  //try to marinate this logic
  const handleSubmit = () => {
    if (!prompt.trim() || !isSignedIn) return;
    //navigate to the dream page with the prompt as a query param
    router.push(`/workspace?prompt= ${encodeURIComponent(prompt.trim())}`)
  }


  //logic for handling enter key , shift key for new lines 
  ...
  //for rottating the placeholder every 3 second
  useEffect(() => {
    if (isFocused || prompt) return; //don't rotate if the user is focused or has typed something
  ...
  useEffect(() => {
    ...
    el.style.height = "auto"; //reset height to auto to get the correct scrollHeight
    el.style.height = Math.min(el.scrollHeight, 200) + "px"; //set height to scrollHeight to fit content

  }, [prompt]) //run this effect whenever the prompt changes to adjust height accordingly

  return (
    <main className="relative isolate min-h-screen overflow-x-hidden bg-[#0a0a0a] selection:bg-white/20">
      <StarsBackground className="pointer-events-none fixed inset-0 -z-10" />

      <section className="relative z-10 flex flex-col items-center overflow-hidden px-4 pb-24 pt-40 text-center">
        <Badge variant={'outline'} className="gap-2 p-4 backdrop-blur-sm">
          <div className="h-1.5 w-1.5 animate-pulse rounded-full bg-emerald-400" />
          Powered by Gemini 3.5 Flash
        </Badge>

        <h1 className="mx-auto max-w-3xl text-balance font-serif text-5xl leading-tight tracking-tight z-10  sm:text-5xl lg:text-7xl ">
          <GrayTitle>Canvas Your Dream </GrayTitle>
          <br />
        ...
        <p className="mx-auto mt-6 max-w-xl text-balance text-base  leading-relaxed text-white/40 z-10">
          Stop wrestling with boilerplate. Describe what you want, and let AI handle the building while you focus on ideas.
        </p>
        ...
        <div className="relative mx-auto mt-12 w-full max-w-2xl">
          <div className={cn("rounded-2xl border bg-[#111111] duration-200",
            ...
            <textarea ref={textareaRef}
              ...
              placeholder={PLACEHOLDERS[placeholder]}  // rotating placeholder from dataset
            />
            <div className="flex items-center justify-between border-t border-white/6 px-4 py-2.5" >
              ...
              {isSignedIn ? (
                <Button
                  ...
                  variant={prompt.trim() ? "default" : "outline"} //change button style based on whether prompt is empty
        ...
        <p className="text-white/20 mt-10 text-lg">
          Type. Generate. Flex.
        </p>
        ...
        <div className="mx-auto grid max-w-5xl grid-cols-1 gap-px overflow-hidden rounded-2xl border border-white/6 bg-white/6 sm:grid-cols-2 lg:grid-cols-3">
          {
            FEATURES.map(({ icon: Icon, label, desc }) => {
              return (
                ...
                >
                  <div className="flex mb-4 h-9 w-9 items-center justify-center rounded-lg border border-white/8 bg-white/4 group-hover:bg-white/8 group-hover:border-white/15">
            ...
            STEPS.map((step, i) => (
              ...
              >
                <div className="flex flex-col items-center">
                  <div className="flex h-10 w-10 items-center justify-center rounded-full norder border-white/10 bg-white/4">
                ...
                <div className="pb-10 pt-1.5">
                  <p className="mb-1.5 text-sm font-semibold sm:text-base">{step.label}
                  </p>
      ...
      <section className="relative mx-auto mb-32 max-w-5xl overflow-hidden rounded-2xl border border-white/8 px-10 py-24 text-center">
        <StarsBackground
          className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_at_bottom,#0d0d0d_0%,#010101_100%)]"
      ...
      <footer className="relative z-10 border-t border-white/7 py-12 mx-auto px-6 flex flex-wrap items-center justify-center text-stone-400">
        Made by developer, for developers. Powered by Gemini 3.5 Flash.
      </footer>
